<?php
$pre_text = 'VG ';
if(function_exists('vc_map')){
    try {
        vc_map(array(
            'name' => esc_html__($pre_text . 'Collection Sign Up - Home', 'crismaster'),
            'base' => 'collection_home_signup',
            'class' => '',
            'icon' => 'icon-st',
            'category' => 'Content',
            'params' => array(
                array(
                    'type' => 'attach_image',
                    'heading' => esc_html__('Hình Ảnh', 'crismaster'),
                    'param_name' => 'image',
                    'value' => '',
                    'description' => esc_html__('Hình ảnh có kích thước 826 x 772', "crismaster")
                ),
                array(
                    'type' => 'textarea_raw_html',
                    'heading' => esc_html__('Nội dung chính','crismaster'),
                    'param_name' => 'des',
                    'value' => '',
                    'description' => esc_html__('Có thể nhập text dạng html',"crismaster")
                ),
            )
        ));
    } catch (Exception $e) {
    }
}
add_shortcode('collection_home_signup','collection_home_signup_func');
function collection_home_signup_func($atts,$content = null){
    extract(shortcode_atts(array(
        'image' => '',
        'des' => '',
    ),$atts));
    ob_start();
    if(isset($image) && $image!='') {
        $image_link = wp_get_attachment_image_src($image, '');
    }
    ?>

    <section class="section-widget section-bgr-02">
        <div class="container-fluiddd">
            <div class="row align-center">
                <div class="col-lg-4 col-md-6 col-sm-12 respon-order-2">
                    <div class="column-content-bgr content-center">
                        <?= isset($des) ? urldecode(base64_decode($des)) : ''; ?>

                    </div>
                </div>
                <div class="col-lg-8 col-md-6 col-sm-12">
                    <div class="column-img-bgr">
                        <img src="<?= esc_url($image_link[0]) ?>" style="width: 100%;transform: translateX(15px);" alt="img-slide">
                    </div>
                </div>
            </div>
        </div>
    </section>
    <?php
    return ob_get_clean();
}
?>